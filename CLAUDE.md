...
- **Never change `main` directly.** All work happens on a dedicated branch, unless the user explicitly says to work on `main` in that same turn. Branch off `main`, do the work there, and leave it for review/merge — don't commit to `main` on your own initiative.
- **Comments only when truly necessary.** Don't add a comment just because it feels like it might help later — only when the WHY is genuinely non-obvious (a hidden constraint, a workaround, a subtle invariant that would surprise a reader). When in doubt, leave the comment out; well-named code should carry the WHAT on its own.
